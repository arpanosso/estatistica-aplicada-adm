...
subtitle: "Distribuição F de Snedecor e Comparação de Parâmetros de duas Populações"
...
**Exemplo**. Duas equipes, cada uma composta por $8$ funcionários de uma empresa, foram submetidas ao mesmo programa de treinamento durante um mês. Os funcionários pertencem a dois setores diferentes da empresa. Ao final do período, foram avaliados os Tempo médio para execução das tarefas (horas):
...
Ao nível de $5\%$, as variâncias dos tempos médios de execução das tarefas diferiram entre si?

---

```{r}
g1<-c(2.30,2.10,1.91,1.20,1.93,1.88,1.95,2.10)
g2<-c(2.30,2.15,2.00,1.28,2.15,2.20,1.91,2.06)
var.test(g1,g2)
```
...
$$
\begin{cases}
H_0: \sigma_{G1}^2 = \sigma_{G2}^2 \\
H_1: \sigma_{G1}^2 \neq \sigma_{G2}^2
\end{cases}
$$
Calculando os valores de variância para os dois grupos de funcionários:
...
sendo que $n_{G1} = n_{G2} = 8$ 

e $\alpha = 5\%$


A estatística do teste: $\frac{s_{G1}^2}{s_{G2}^2} = \frac{0{,}10433}{0{,}10068} = 1,03618$
...
Como $F_{obs} \notin RC$ não se rejeita $H_0$, ou seja, as variâncias são estatisticamente iguais ao nível de $5\%$ de significância, ou seja, as variâncias dos tempos de realização das tarefas são homocedásticas.
...
Assim, $t = \frac{(\bar{X} - \bar{Y})}{s_p\sqrt{\frac{1}{n_1}+\frac{1}{n_2}}} \sim t(GL = n_1+n_2 - 2)$
---
...
Ao nível de $5\%$, as médias dos tempos de execução de tarefas diferem entre si?
...
g1<-c(2.30,2.10,1.91,1.20,1.93,1.88,1.95,2.10)
g2<-c(2.30,2.15,2.00,1.28,2.15,2.20,1.91,2.06)
t.test(g1, g2, alternative = "le", var.equal = TRUE)
```

---

Usando os dados do exemplo anterior, testar se há evidência de que os dois grupos des funcionários apresentam o mesmo tempo de execução de tarefas $(H_0: \mu_A = \mu_B \text{ vs. } H_1: \mu_A < \mu_B)$, ao nível de $5\%$.
...
$\bar{X} = 1{,}92125$ e $s_{G1}^2 = 0{,}10433$ e  $\bar{Y} = 2{,}00625$ e $s_{G2}^2 = 0{,}10068$

sendo que $n_{G1} = n_{G2} = 8$ e $\alpha = 5\%$
...
---
**Conclusão**: Como $t_{obs} \notin RC$, não rejeita-se $H_0$, não há evidências de que o grupo de funcionários 1 apresenta maior tempo em execução de tarefas que o grupo 2.
