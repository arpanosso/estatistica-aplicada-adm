...
Até agora aprendemos a construir o intervalo de confiância para a média e para proporção populacional. Agora, serão apresentados métodos para estimar desvios-padrão e variâncias populacionais.

Comecemos com intervalorde confiançã para $\sigma$ baseados na estatística amostral $s$, que exigem que a população da qual extraímos as amostras tenha aproximadamente a forma de uma distribuilção normal. Nesse caso a estatística 
...
Assim como nas distribuições $z_\alpha$ e $t_\alpha$, $\chi^2_\alpha$ é o valor para o qual a área sob a curva e à sua direita é $\alpha$, cujo valor depende do número de graus de liberdade (Tabela).
...
Essa ressalva é necessário, pois a distribuição não é simétrica.
...
---
**Exemplo**: Dado $9$ graus de liberdade e $\alpha$ de $5\%$, o valor de $x$ cuja área após dele é $\alpha$ será $\chi^2_{0{,}05} = 16{,}919$.
...
Semelhante ao realizado para construção dos intervalos e confiança par aa média, temo que $1-\alpha$ é a probabilidade da variáveis aleatória com uma distribuição $\chi^2$ assuma um valor entre:
